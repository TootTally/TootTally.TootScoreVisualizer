...
namespace TootTally.TootScoreVisualizer
{
    public static class TSVConfig
    {
        ...
        public static void LoadConfig(string configName)
        {
            ...
            {
                Plugin.Instance.LogError($"TSV config {configFileName} not found, using default config");
                configFileName = "Default.xml";
                Plugin.options.TSVName.Value = "Default";
                if (!File.Exists(folderPath + configFileName))
                {
                    Plugin.Instance.LogError($"Couldn't find {configFileName}. Generating new Default config");
